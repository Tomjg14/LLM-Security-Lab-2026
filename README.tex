...
\title{LLM Security Lab: Prompt Injection Defenses}
...
\begin{enumerate}[label=\arabic*.]
    \item \textbf{Docker Desktop}: This lab uses \texttt{docker-compose} to manage the application and its dependencies. Download and install Docker Desktop from \url{docker.com}.
    \item \textbf{Ollama}: We will be running a local LLM using Ollama. Download and install Ollama from \url{ollama.com}.
    \begin{itemize}
        \item After installation, pull a model. For example: \texttt{llama3.2}. Open your terminal and run:
\begin{Verbatim}[commandchars=\\\{\}]
        \textbf{ollama} pull llama3.2
\end{Verbatim}
\end{enumerate}
...
\end{Verbatim}
    ...
    \begin{itemize}
        \item Copy the example environment file:
\begin{Verbatim}[commandchars=\\\{\}]
        \textbf{cp} .env.example .env
\end{Verbatim}
        ...
        \begin{itemize}
            ...
            \item Go to \url{https://chat.science.ru.nl/} and log in using your science credentials.
...
As a "defender," your job is to modify the \texttt{sanitize\_input} function in \texttt{defense.py} to detect and neutralize these malicious prompts before they reach the LLM. Experiment with different injection techniques and see if your defenses hold up!
